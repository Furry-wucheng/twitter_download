...
* **2026-06-08**
  ...
  * 缓存数据库记录更多元数据：推文时间、作者、类型、推文内容等
  * 新增缓存迁移工具 `migrate_cache.py`，用于将旧的 `cache_data.log` 迁移到新格式
  * 支持多配置文件选择：启动时可选择不同的 `settings*.json` 配置文件
  * 文件名包含作者名：格式改为 `{时间}_@{作者名}-vid/img`
  * 支持共享缓存：`share_cache` 配置项，多用户可共享同一缓存数据库
  * 新增 `.gitignore` 文件
  * 优化 URL 参数处理，避免同一资源因参数不同被重复下载
